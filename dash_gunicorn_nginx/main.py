...
def verify_id_token(id_token: str) -> dict:
    # Do NOT use _jwks_client.get_signing_key_from_jwt() — it internally
    # calls decode_complete() with leeway=0 and validates iat before we can
    # apply our own leeway, causing ImmatureSignatureError on WSL where the
    # Node.js clock and the Python clock can differ by a few seconds.
    # Instead: extract the kid from the unverified header manually, fetch
    # the matching key, then do our own decode with leeway.
